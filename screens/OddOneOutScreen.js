...
const QUESTIONS = [
  { items: ['🐶','🐱','🐰','🍎'],    odd: '🍎',  reason: 'Die anderen sind Tiere'     },
  { items: ['🍎','🍌','🍊','🚗'],    odd: '🚗',  reason: 'Die anderen sind Früchte'   },
  { items: ['🔴','🔵','🟡','🐶'],    odd: '🐶',  reason: 'Die anderen sind Farben'    },
  { items: ['✈️','🚗','🐟','🚢'],    odd: '🐟',  reason: 'Die anderen sind Fahrzeuge' },
  { items: ['📚','✏️','🐱','📏'],    odd: '🐱',  reason: 'Die anderen sind Schulsachen'},
  { items: ['🌸','🌻','🌺','🍕'],    odd: '🍕',  reason: 'Die anderen sind Blumen'    },
  { items: ['🍕','🍔','🌮','⭐'],    odd: '⭐',  reason: 'Die anderen sind Essen'     },
  { items: ['🎸','🎹','🎺','🍌'],    odd: '🍌',  reason: 'Die anderen sind Instrumente'},
  { items: ['🐘','🦒','🦁','🚕'],    odd: '🚕',  reason: 'Die anderen sind Tiere'     },
  { items: ['⚽','🏀','🎾','🍔'],    odd: '🍔',  reason: 'Die anderen sind Bälle'     },
  { items: ['🌙','☀️','⭐','🐱'],    odd: '🐱',  reason: 'Die anderen sind am Himmel' },
  { items: ['👕','👖','👗','🐶'],    odd: '🐶',  reason: 'Die anderen sind Kleidung'  },
];
...
export default function OddOneOutScreen({ navigation }) {
  ...
  const [usedIdxs]          = useState(() => {
    const arr = Array.from({ length: QUESTIONS.length }, (_, i) => i);
    return arr.sort(() => Math.random() - 0.5);
  });

  const q = QUESTIONS[usedIdxs[qIdx % QUESTIONS.length]];

  useEffect(() => { loadQ(); }, []);

  function loadQ() {
    const curr = QUESTIONS[usedIdxs[qIdx % QUESTIONS.length]];
  ...
  function handlePick(item) {
    ...
    if (item === q.odd) {
      setStatus('correct');
      setScore(s => s + 1);
      speak(`Richtig! ${q.reason}.`, { rate: 0.82, pitch: 1.2 });
      setTimeout(() => {
        if (qIdx + 1 >= TOTAL) setStatus('done');
        else { setQIdx(i => i + 1); loadQ(); }
      }, 2000);
    } else {
      setStatus('wrong');
      speak('Schau nochmal genau hin! Was ist anders?', { rate: 0.82, pitch: 1.1 });
      setTimeout(() => { setStatus('playing'); setPicked(null); }, 1400);
    }
  }
  ...
  return (
    <SafeAreaView style={s.safe}>
      ...
      <Text style={s.score}>⭐ {score}  Runde {qIdx + 1}/{TOTAL}</Text>
      ...
      <View style={s.grid}>
        {items.map((item, i) => {
          ...
          let bg = '#fff';
          if (isPicked && status === 'correct') bg = '#B5EAD7';
          if (isPicked && status === 'wrong')   bg = '#FFD5D5';
          if (status === 'correct' && isOdd && picked !== item) bg = '#B5EAD7';
          return (
            <TouchableOpacity key={i} style={[s.card, { backgroundColor: bg }]}
              onPress={() => handlePick(item)} activeOpacity={0.75}>
              <Text style={s.cardTxt}>{item}</Text>
              {status === 'correct' && isOdd && <Text style={s.checkmark}>✓</Text>}
            </TouchableOpacity>
          );
        })}
      </View>
...
const s = StyleSheet.create({
  ...
  hint:       { fontSize: 16, color: '#888', marginTop: 4, marginBottom: 16, textAlign: 'center', paddingHorizontal: 20 },
  grid:       { flexDirection: 'row', flexWrap: 'wrap', gap: 16, justifyContent: 'center', paddingHorizontal: 20 },
  card:       { width: CARD, height: CARD, borderRadius: 24, alignItems: 'center', justifyContent: 'center', elevation: 4, shadowColor: '#000', shadowOpacity: 0.1, shadowRadius: 8, shadowOffset: { width: 0, height: 3 } },
  cardTxt:    { fontSize: IS_TABLET ? 72 : 58 },
  checkmark:  { fontSize: 22, color: '#52B788', fontWeight: '900', position: 'absolute', top: 8, right: 12 },
  speakBtn:   { marginTop: 20, backgroundColor: '#E8F4FD', paddingVertical: 10, paddingHorizontal: 24, borderRadius: 20 },
